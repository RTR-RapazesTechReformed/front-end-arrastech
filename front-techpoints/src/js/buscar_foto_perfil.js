...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const containerNomeUsuario = document.querySelector(
    ".container_nome_usuario"
  ); // Seleciona o contêiner pelo seletor de classe
  ...
  if (profileImageHeader) {
    async function fetchProfileImage() {
      ...
      try {
        const response = await fetch(`${backendUrl}/usuarios/imagem/${userId}`);
        if (response.ok) {
