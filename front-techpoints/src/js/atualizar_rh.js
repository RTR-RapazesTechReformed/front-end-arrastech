...
async function salvarMudancas(event) {
  ...
  const dados = {};
  dados.nomeUsuario = `${novoPrimeiroNome || data.primeiroNome} ${
    novoSobrenome || data.sobrenome
  }`.trim();
  ...
  const endpoint = `${backendUrl}/usuarios/atualizar/${userId}`;
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  async function listarFavoritos() {
    ...
    try {
      const response = await fetch(
        `${backendUrl}/dashboardRecrutador/${idUsuario}/listar/favoritos`
      );
      ...
      if (data.length === 0) {
        ...
        for (const aluno of data) {
          const pontosResponse = await fetch(
            `${backendUrl}/pontuacoes/pontos-totais/${aluno.id}`
          );
  ...
  async function listarInteressados() {
    ...
    try {
      const response = await fetch(
        `${backendUrl}/dashboardRecrutador/${idUsuario}/listar/interessados`
      );
      const data = await response.json();
      ...
      if (data.length === 0) {
        ...
        noInterestedMessage.textContent =
          "Não há alunos interessados no momento.";
  ...
  async function listarCancelados() {
    ...
    try {
      const response = await fetch(
        `${backendUrl}/dashboardRecrutador/${idUsuario}/listar/cancelados`
      );
      const data = await response.json();
      ...
      if (data.length === 0) {
        const noInterestedMessage = document.createElement("p");
        noInterestedMessage.textContent =
          "Não há alunos cancelados no momento.";
        noInterestedMessage.className = "no-alunos-message";
        container.appendChild(noInterestedMessage);
        ...
        const desinteressados = JSON.parse(
          localStorage.getItem("desinteressados") || "[]"
        );

        for (const alunoDiv of alunoDivs) {
          const alunoId = alunoDiv.querySelector("span").textContent.trim();
          const aluno = data.find(
            (a) => `${a.primeiroNome} ${a.sobrenome}` === alunoId
          );

          const isFavorito = favoritos.some((f) => f.id === aluno.id);
          const isDesinteressado = desinteressados.some(
            (d) => d.id === aluno.id
          );

          if (!isFavorito && !isDesinteressado) {
            const response = await fetch(
              `${backendUrl}/dashboardRecrutador/${idUsuario}/contratados/${aluno.id}`,
              {
                method: "DELETE",
              }
            );
  ...
  window.desfazer = async function (idAluno) {
    ...
    let desinteressados =
      JSON.parse(localStorage.getItem("desinteressados")) || [];

    const alunoFavoritosIndex = favoritos.findIndex(
      (favorito) => Number(favorito.id) === Number(idAluno)
    );
    const alunoDesinteressadosIndex = desinteressados.findIndex(
      (interessado) => Number(interessado.id) === Number(idAluno)
    );
    ...
    try {
      ...
      if (alunoFavoritosIndex !== -1) {
        response = await fetch(
          `${backendUrl}/dashboardRecrutador/${getIdUsuarioLogado()}/favoritos/${idAluno}`,
          {
            method: "POST",
          }
        );
        ...
        if (response.ok) {
          ...
          console.error(
            "Erro ao restaurar aluno dos favoritos:",
            response.statusText
          );
        }
        ...
        response = await fetch(
          `${backendUrl}/dashboardRecrutador/${getIdUsuarioLogado()}/${listaAssociada}/${idAluno}`,
          {
            method: "POST",
          }
        );
        ...
        if (response.ok) {
          ...
          localStorage.setItem(
            "desinteressados",
            JSON.stringify(desinteressados)
          );
          showAlert("Aluno restaurado com sucesso!", "success");
          setTimeout(() => {
            location.reload();
          ...
          console.error(
            "Erro ao restaurar aluno dos desinteressados:",
            response.statusText
          );
        }
      } else {
  ...
  window.desfavoritar = async function (idAluno) {
    ...
    try {
      const response = await fetch(
        `${backendUrl}/dashboardRecrutador/${idUsuario}/cancelados/${idAluno}`,
        {
          method: "POST",
        }
      );
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  if (user) {
    document.getElementById("id_nome").innerText =
      `${user.primeiroNome} ${user.sobrenome}` || "Nome não disponível";
    document.getElementById("id_email_usuario").innerText =
      user.email || "Email não disponível";
    document.getElementById("span_data_criacao").innerText = user.dataCriacao
      ? new Date(user.dataCriacao).toLocaleDateString()
      : "Data não disponível";
    document.getElementById("span_data_ultima_atualizacao").innerText =
      user.dataAtualizacao
        ? new Date(user.dataAtualizacao).toLocaleDateString()
        : "Não houve nenhuma atualização";
    document.getElementById("span_empresa").innerText =
      user.empresa.nome || "Nome da empresa não disponível";
    document.getElementById("span_setor").innerText =
      user.empresa.setorIndustria || "Setor não disponível";
    document.getElementById("span_cargo").innerText =
      user.cargoUsuario || "Email não disponível";
    document.getElementById("span_email_empresa").innerText =
      user.empresa.emailCorporativo || "Email não disponível";
  } else {
    document.getElementById("id_nome").innerText = "Nome não disponível";
    document.getElementById("id_email_usuario").innerText =
      "Email não disponível";
    document.getElementById("span_data_criacao").innerText =
      "Data não disponível";
    document.getElementById("span_data_ultima_atualizacao").innerText =
      "Não houve nenhuma atualização";
  }
...
function editarPerfil() {
  document.querySelector(".container_fundo_editar_informacoes").style.display =
    "flex";
}
...
function atribuir(idAluno, fromListar = false, elementoChamador = null) {
  ...
  if (fromListar) {
    const boxProcessoSeletivo = document.querySelector(
      ".box_atribuir:nth-child(3)"
    );
  ...
  btnDesinteressar.onclick = () =>
    desinteressarAluno(idAlunoSelecionado, elementoChamador);
}
...
async function interessarAluno(idAlunoSelecionado) {
  ...
  try {
    const response = await fetch(
      `${backendUrl}/dashboardRecrutador/${idRecrutador}/interessados/${idAlunoSelecionado}`,
      {
        method: "POST",
      }
    );
...
async function contratarAluno(idAluno) {
  ...
  try {
    const response = await fetch(
      `${backendUrl}/dashboardRecrutador/${idRecrutador}/contratados/${idAluno}`,
      {
        method: "POST",
      }
    );
    ...
    if (response.ok) {
      ...
      const contratadosData = await fetch(
        `${backendUrl}/dashboardRecrutador/${idRecrutador}/listar/contratados`
      );
...
async function processoSeletivoAluno(idAluno) {
  ...
  try {
    const response = await fetch(
      `${backendUrl}/dashboardRecrutador/${idRecrutador}/processoSeletivo/${idAluno}`,
      {
        method: "POST",
      }
    );
    ...
    if (response.ok) {
      ...
      const processoSeletivoData = await fetch(
        `${backendUrl}/dashboardRecrutador/${idRecrutador}/listar/processoSeletivo`
      );
      const processoSeletivo = await processoSeletivoData.json();
      sessionStorage.setItem(
        "quantidadeProcessoSeletivo",
        processoSeletivo.length
      );
...
async function listarProcessoSeletivo() {
  ...
  try {
    ...
    if (data.length === 0) {
      const noInterestedMessage = document.createElement("p");
      noInterestedMessage.textContent =
        "Não há alunos no processo seletivo no momento.";
      noInterestedMessage.className = "no-alunos-message";
      container.appendChild(noInterestedMessage);
    } else {
      for (const aluno of data) {
...
async function desinteressarAluno(idAluno, elementoChamador) {
  ...
  try {
    const response = await fetch(
      `${backendUrl}/dashboardRecrutador/${idRecrutador}/cancelados/${idAluno}`,
      {
        method: "POST",
      }
    );
