...
function verificaEmail() {
  ...
  const botaoProximo = document.querySelector(
    'form#formEmail button[type="submit"]'
  ...
  if (email) {
    ...
    fetch(`${backendUrl}/reset-senha/solicitar-troca`, {
      method: "POST",
      headers: {
      ...
      .then((response) => {
        if (response.ok) {
          ...
          return response.json().then((errorData) => {
            showAlert(
              errorData.message || "Falha ao enviar o código. Tente novamente.",
              "error"
            );
          });
        }
      ...
      .catch(() => {
        showAlert(
          "Erro ao tentar se conectar com o servidor. Tente novamente.",
          "error"
        );
      })
      .finally(() => {
...
function verificaToken() {
  ...
  if (codigo && email) {
    loader.style.display = "flex";
    fetch(`${backendUrl}/reset-senha/verificar-token`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      ...
      .then((response) => {
        if (response.ok) {
          mostrarCamposSenha();
          showAlert(
            "Código verificado com sucesso. Agora, crie uma nova senha.",
            "success"
          );
        } else {
          return response.json().then((errorData) => {
...
function validarSenha() {
  ...
  fetch(`${backendUrl}/reset-senha/nova-senha`, {
    method: "PATCH",
    headers: {
      "Content-Type": "application/json",
    },
    ...
    .then((response) => {
      if (response.ok) {
        ...
        return response.json().then((errorData) => {
          showAlert(
            errorData.message || "Falha ao atualizar a senha. Tente novamente.",
            "error"
          );
        });
      }
