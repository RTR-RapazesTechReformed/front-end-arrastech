...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (filtroCursos && tabelaRanking) {
    async function buscarEExibirRanking() {
      ...
      try {
        const response = await fetch(`${backendUrl}/pontuacoes/ranking`);
        ...
        const arrayRanking = Object.values(pontosTotais).sort(
        ...
        arrayRanking.forEach((entrada, index) => {
          ...
          if (index === 0) {
            medalhaHtml =
              '<img src="/imgs/gold_medal.png" alt="Medalha de Ouro" style="width: 40px; height: 40px;">';
          } else if (index === 1) {
            medalhaHtml =
              '<img src="/imgs/silver_medal.png" alt="Medalha de Prata" style="width: 40px; height: 40px;">';
          } else if (index === 2) {
            medalhaHtml =
              '<img src="/imgs/bronze_medal.png" alt="Medalha de Bronze" style="width: 40px; height: 40px;">';
          } else {
    ...
    async function carregarImagemPerfil(id) {
      ...
      loader.style.display = "flex";
      try {
        const response = await fetch(`${backendUrl}/usuarios/imagem/${id}`);
        if (response.ok) {
    ...
    async function popularFiltroCategorias() {
      ...
      try {
        const response = await fetch(
          `${backendUrl}/dashboardRecrutador/listar`
        );
        if (!response.ok)
          throw new Error("Falha ao buscar as categorias dos cursos.");
    ...
    async function carregarCursosPorCategoria(categoria) {
      ...
      try {
        ...
        if (!response.ok)
          throw new Error("Falha ao buscar cursos da categoria selecionada.");

        const cursos = await response.json();

        cursos.forEach((curso) => {
          curso.categorias = [
            ...new Set(curso.categorias.split(",").map((c) => c.trim())),
          ];
        });
    ...
    filtroCursos.addEventListener("change", function () {
      ...
      if (categoriaSelecionada !== "all") {
        ...
        filtroCurso.innerHTML =
          '<option value="all">Selecione uma categoria primeiro</option>';
    ...
    document
      ...
      .addEventListener("change", async function () {
        loader.style.display = "flex";
        try {
          const categoriaSelecionadaNome = this.options[this.selectedIndex].text
            .trim()
            .toLowerCase();
          ...
          const chaveCategoria = `cursos_${
            categoriaSelecionadaNome.charAt(0).toUpperCase() +
            categoriaSelecionadaNome.slice(1)
          }`; // Exemplo: cursos_Tecnologia
          const cursosDaCategoria = JSON.parse(
            sessionStorage.getItem(chaveCategoria)
          );
          ...
          const responseRanking = await fetch(
            `${backendUrl}/pontuacoes/ranking`
          );
          if (!responseRanking.ok)
            throw new Error("Falha ao buscar o ranking dos cursos.");
          ...
          cursosDaCategoria.forEach((curso) => {
            ...
            const dadosCurso = Object.values(dadosRanking).find(
              (rankingCurso) =>
                rankingCurso.nomeCurso.trim().toLowerCase() ===
            ...
            if (
              ...
              dadosCurso.ranking.forEach((entrada, index) => {
                ...
                let pontosSoma = sessionStorage.getItem(
                  `pontosTotais_${alunoId}`
                );
                if (!pontosSoma) {
                  pontosSoma = 0;
                }
    ...
    document
      ...
      .addEventListener("change", async function () {
        ...
        try {
          const cursoSelecionadoNome = this.options[this.selectedIndex].text
            .trim()
            .toLowerCase();
          ...
          if (cursoSelecionadoNome === "selecione um curso") {
            ...
            const response = await fetch(`${backendUrl}/pontuacoes/ranking`);
            if (!response.ok)
              throw new Error("Falha ao buscar o ranking por curso.");

            const dados = await response.json();

            const dadosCurso = Object.values(dados).find(
              (curso) =>
