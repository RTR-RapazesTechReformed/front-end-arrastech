...
async function carregarCategorias() {
  ...
  try {
    const response = await fetch(`${backendUrl}/dashboardRecrutador/listar`);
    ...
    categoriasSelect.innerHTML =
      '<option value="opc_categorias">Categorias</option>';
...
async function exibirCursosPorCategoria() {
  ...
  try {
    const response = await fetch(
      `${backendUrl}/dashboardRecrutador/listar?categoria=${categoria}`
    );
    const data = await response.json();
...
async function exibirAlunosDoCurso(curso) {
  loader.style.display = "flex";
  try {
    const alunosResponse = await fetch(`${backendUrl}/pontuacoes/alunos`);
    ...
    const interessadosResponse = await fetch(
      `${backendUrl}/dashboardRecrutador/${idUsuarioLogado}/listar/interessados`
    );
    const interessadosData = await interessadosResponse.json();
    const idsInteressados = interessadosData.map((aluno) => aluno.id);

    const processoSeletivoResponse = await fetch(
      `${backendUrl}/dashboardRecrutador/${idUsuarioLogado}/listar/processoSeletivo`
    );
    ...
    const contratadosResponse = await fetch(
      `${backendUrl}/dashboardRecrutador/${idUsuarioLogado}/listar/contratados`
    );
    ...
    const favoritosResponse = await fetch(
      `${backendUrl}/dashboardRecrutador/${idUsuarioLogado}/listar/favoritos`
    );
    ...
    if (alunosExibidos === 0) {
      mensagemDiv.textContent =
        "Todos os alunos deste curso já foram visualizados.";
      mensagemDiv.style.display = "flex";
    }
  } catch (error) {
...
async function atualizarAlunos() {
  ...
  try {
    ...
    const cursosResponse = await fetch(`${backendUrl}/pontuacoes/alunos`);
    ...
    for (let i = 0; i < cursosEmbaralhados.length; i++) {
      ...
      if (!blocoAlunos) {
        console.error(
          `Bloco de alunos para o curso ${cursoId} não encontrado.`
        );
        continue;
      }
      ...
      for (const aluno of alunos) {
        if (
          !idsInteressados.includes(aluno.aluno.id) &&
      ...
      if (alunosParaExibir.length > 0) {
        ...
        if (alunosExibidos.length === 0) {
          ...
          mensagem.innerText =
            "Todos os alunos desse curso já foram visualizados.";
...
async function verMais(alunoId) {
  ...
  try {
    ...
    const response = await fetch(`${backendUrl}/usuarios/buscar/${alunoId}`);
    ...
    document.getElementById("municipio_do_aluno").innerText =
      aluno.endereco.cidade;
    document.getElementById("escolaridade_do_aluno").innerText =
      aluno.escolaridade;
    ...
    const fotoResponse = await fetch(
      `${backendUrl}/usuarios/imagem/${alunoId}`
    );
    if (fotoResponse.ok) {
    ...
    const emblemasResponse = await fetch(
      `${backendUrl}/pontuacoes/pontos-totais/${alunoId}`
    );
    ...
    const containerBotaoInteresse = document.querySelector(
      ".bloco_botao_interesse"
    );
    ...
    const alunoFavoritado = favoritosData.some(
      (favorito) => favorito.id === alunoId
    );
    ...
    interesseButton.onclick = () =>
      tenhoInteresse(alunoId, interesseButton, nomeAluno);
...
async function favoritar(alunoId, favoritarButton) {
  ...
  try {
    const response = await fetch(
      `${backendUrl}/dashboardRecrutador/${idUsuarioLogado}/favoritos/${alunoId}`,
      {
        method: "POST",
      }
    );
    ...
    if (response.ok) {
      ...
      const boxAluno = document.querySelector(
        `.box_aluno[data-aluno-id="${alunoId}"]`
      );
      if (boxAluno) {
...
async function tenhoInteresse(alunoId, interesseButton, nomeAluno) {
  ...
  try {
    const response = await fetch(
      `${backendUrl}/dashboardRecrutador/${idUsuarioLogado}/interessados/${alunoId}`,
      {
        method: "POST",
      }
    );
...
document.addEventListener("DOMContentLoaded", async function () {
  loader.style.display = "flex";
  try {
    const response = await fetch(`${backendUrl}/usuarios/listar`);
    const data = await response.json();
...
async function exibirAlunosPorCidade(cidadeSelecionada) {
  ...
  try {
    const [
      pontuacoesResponse,
      usuariosResponse,
      interessadosResponse,
      contratadosResponse,
      processoSeletivoResponse,
      favoritosResponse,
    ] = await Promise.all([
      fetch(`${backendUrl}/pontuacoes/ranking`),
      fetch(`${backendUrl}/usuarios/listar`),
      fetch(
        `${backendUrl}/dashboardRecrutador/${getIdUsuarioLogado()}/listar/interessados`
      ),
      fetch(
        `${backendUrl}/dashboardRecrutador/${getIdUsuarioLogado()}/listar/contratados`
      ),
      fetch(
        `${backendUrl}/dashboardRecrutador/${getIdUsuarioLogado()}/listar/processoSeletivo`
      ),
      fetch(
        `${backendUrl}/dashboardRecrutador/${getIdUsuarioLogado()}/listar/favoritos`
      ),
    ]);
    ...
    const interessadosSet = new Set(
      interessadosData.map((interessado) => interessado.id)
    );
    const contratadosSet = new Set(
      contratadosData.map((contratado) => contratado.id)
    );
    const processoSeletivoSet = new Set(
      processoSeletivoData.map((aluno) => aluno.id)
    );
    const favoritosSet = new Set(favoritosData.map((favorito) => favorito.id));

    const alunosPorCurso = Object.entries(pontuacoesData).reduce(
      (acc, [cursoId, curso]) => {
        const alunosFiltrados =
          curso.ranking?.filter((aluno) => {
            const alunoId = aluno.aluno.id;
            const cidadeDoAluno = usuariosMap[alunoId]?.cidade;

            return (
              cidadeDoAluno &&
              cidadeDoAluno.toLowerCase() ===
                cidadeSelecionada.toLowerCase().replace(/_/g, " ") &&
    ...
    Object.entries(alunosPorCurso).forEach(([curso, alunos], index) => {
      ...
      alunos.forEach((aluno) => {
        ...
        alunoDiv.className =
          "box_Aluno" +
          (favoritosSet.has(aluno.aluno.id) ? " aluno-favorito" : "");
        alunoDiv.innerHTML = `
                    <span>${aluno.aluno.primeiroNome} ${aluno.aluno.sobrenome}</span>
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  try {
    ...
    escolaridadeSelect.innerHTML =
      '<option value="opc_escolaridade">Escolaridade</option>';
...
async function exibirAlunosPorEscolaridade(escolaridadeSelecionada) {
  ...
  try {
    ...
    const pontuacoesResponse = await fetch(`${backendUrl}/pontuacoes/ranking`);
    const pontuacoesData = await pontuacoesResponse.json();

    const usuariosResponse = await fetch(`${backendUrl}/usuarios/listar`);
    ...
    const favoritosSet = Array.isArray(favoritosData)
      ? new Set(favoritosData.map((aluno) => aluno.id))
      : new Set();
    ...
    for (const cursoId in pontuacoesData) {
      ...
      const alunosFiltrados = alunos.filter((aluno) => {
        const alunoId = aluno.aluno.id;
        const escolaridadeDoAluno = usuariosMap[alunoId]?.escolaridade
          ?.toLowerCase()
          .replace(/\s+/g, "_");

        return (
          escolaridadeDoAluno ===
            escolaridadeSelecionada.toLowerCase().replace(/\s+/g, "_") &&
          !contratadosSet.has(alunoId) &&
          !interessadosSet.has(alunoId) &&
          !processoSeletivoSet.has(alunoId)
        );
  ...
  try {
    ...
    for (const cursoId in pontuacoesData) {
      ...
      const alunosFiltradosPorNome = alunos.filter((aluno) => {
        ...
        return (
          nomeCompleto.toLowerCase().includes(nomeBuscado.toLowerCase()) &&
          !interessadosSet.has(alunoId) &&
          !contratadosSet.has(alunoId) &&
          !processoSeletivoSet.has(alunoId)
        );
      ...
      if (alunosFiltradosPorNome.length > 0) {
        alunosFiltrados.push(
          ...alunosFiltradosPorNome.map((aluno) => ({
            ...aluno,
            curso: curso.nomeCurso,
          }))
        );
...
async function exibirAlunosPorSexo(sexoSelecionado) {
  ...
  try {
    ...
    const processoSeletivoSet = new Set(
      processoSeletivoData.map((processo) => processo.id)
    );
