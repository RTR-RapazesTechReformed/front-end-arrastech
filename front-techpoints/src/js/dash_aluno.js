document.addEventListener("DOMContentLoaded", async function () {
  ...
  if (user && user.idUsuario) {
    try {
      const response = await fetch(
        `${backendUrl}/usuarios/imagem/${user.idUsuario}`
      );
      if (response.ok) {
  ...
  try {
    const response = await fetch(
      `${backendUrl}/usuarios/buscar/${user.idUsuario}`
    );
    if (!response.ok) {
...
document
  ...
  .addEventListener("submit", async function (event) {
    ...
    if (file) {
      ...
      try {
        const response = await fetch(
          `${backendUrl}/usuarios/atualizar-imagem/${user.idUsuario}`,
          {
            method: "PATCH",
            body: file,
            headers: {
...
document
  ...
  .addEventListener("submit", async function (event) {
    ...
    try {
      const response = await fetch(
        `${backendUrl}/usuarios/atualizar/${user.idUsuario}`,
        {
          method: "PATCH",
          headers: {
