...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    .getElementById("selectEmpresas")
    .addEventListener("change", function () {
      renderizarGraficos();
    });
  document
    .getElementById("selectListas")
    .addEventListener("change", function () {
      renderizarGraficos();
    });

  document
    .getElementById("baixarRelatorio")
    .addEventListener("click", gerarRelatorioAlunos);
...
async function listarEmpresas() {
  try {
    const resposta = await fetch(`${backendUrl}/empresa/listar`);
    if (!resposta.ok) {
...
async function buscarDados() {
  try {
    ...
    let url = `${backendUrl}/dashboardAdm/demografia-alunos?tipoLista=${tipoLista}`;
...
async function gerarRelatorioEmpresas() {
  try {
    ...
    let url = `${backendUrl}/dashboardAdm/relatorio-demografia-listas?tipoLista=${tipoLista}`;
...
async function gerarRelatorioAlunos() {
  try {
    ...
    let url = `${backendUrl}/dashboardAdm/relatorio-alunos?`;
...
async function enviarArquivo() {
  ...
  try {
    const response = await fetch(
      `${backendUrl}/dashboardAdm/cadastrar-alunos-lote`,
      {
        method: "POST",
        body: formData,
      }
...
async function renderizarGraficos() {
  ...
  if (dadosRecebidos) {
    ...
    const ctxSexo = document
      .getElementById("distribuicaoPorSexoListas")
      .getContext("2d");
    graficoSexo = criarGraficoRosca(
      ctxSexo,
      rotulosSexo,
      dadosSexo,
    ...
    const ctxEtnia = document
      .getElementById("distribuicaoPorEtniaListas")
      .getContext("2d");
    graficoEtnia = criarGraficoRosca(
      ctxEtnia,
      rotulosEtnia,
      dadosEtnia,
    ...
    const ctxEscolaridade = document
      .getElementById("distribuicaoPorEscolaridadeListas")
      .getContext("2d");
    graficoEscolaridade = criarGraficoRosca(
      ctxEscolaridade,
      rotulosEscolaridade,
      dadosEscolaridade,
    ...
    const ctxCidade = document
      .getElementById("alunosPorCidade")
      .getContext("2d");
    const coresCidade = gerarCores(rotulosCidade.length);
    graficoCidade = criarGraficoBarras(
      ctxCidade,
      rotulosCidade,
      dadosCidade,
    ...
    graficoCursos = criarGraficoBarras(
      ctxCursos,
      rotulosCursos,
      dadosCursos,
      "Distribuição de Cursos Feitos",
      coresCursos
    );
  } else {
