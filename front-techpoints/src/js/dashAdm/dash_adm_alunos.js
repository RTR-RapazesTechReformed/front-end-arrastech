...
async function carregarAlunos() {
  try {
    const response = await fetch(`${backendUrl}/usuarios/listar?tipo=Aluno`);
    if (response.ok) {
      ...
      alunos.forEach((aluno) => {
        ...
        dataNascimentoCell.textContent = new Date(
        ...
        sexoData[aluno.sexo]
          ? sexoData[aluno.sexo]++
          : (sexoData[aluno.sexo] = 1);
        etniaData[aluno.etnia]
          ? etniaData[aluno.etnia]++
          : (etniaData[aluno.etnia] = 1);
      });
...
async function carregarAlunosPorCurso() {
  try {
    const response = await fetch(`${backendUrl}/dashboardAdm/alunos-por-curso`);
    if (response.ok) {
      const cursosData = await response.json();

      const nomeCursos = cursosData.map((curso) => curso.nomeCurso);
      const quantidadeAlunos = cursosData.map(
        (curso) => curso.quantidadeAlunos
      );
...
function gerarGraficos(sexoData, etniaData) {
  const ctxSexo = document
    .getElementById("distribuicaoPorSexo")
    .getContext("2d");
  new Chart(ctxSexo, {
    type: "doughnut",
    data: {
  ...
  const ctxEtnia = document
    .getElementById("distribuicaoPorEtnia")
    .getContext("2d");
  new Chart(ctxEtnia, {
    type: "doughnut",
    data: {
...
function filtrarAlunos() {
  const selectEscolaridade = document.getElementById(
    "select-escolaridade"
  ).value;
  ...
  const inputIdadeMax =
    parseInt(document.getElementById("idade-max").value) || 150;
...
document
  .getElementById("select-escolaridade")
  .addEventListener("change", filtrarAlunos);
document
  .getElementById("select-cidade")
  .addEventListener("change", filtrarAlunos);
document
  .getElementById("select-sexo")
  .addEventListener("change", filtrarAlunos);
document
  .getElementById("select-etnia")
  .addEventListener("change", filtrarAlunos);
