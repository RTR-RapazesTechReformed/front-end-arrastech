...
function validarSenha(senhaInput, confirmacaoSenhaInput) {
  ...
  return senhaValue !== confirmacaoSenhaValue
...
async function salvarMudancas(event) {
  ...
  if (
    ...
    senhaConfirmacaoInput.value.trim() !== ""
  ) {
  ...
  if (
    ...
    if (
      ...
      erros.push(
        "Todos os campos de endereço (CEP, Estado, Cidade, Rua) devem estar preenchidos."
      );
    } else {
  ...
  if (
    ...
    Object.keys(enderecoUpdates).length === 0
  ) {
  ...
  try {
    if (Object.keys(updates).length > 0) {
      const response = await fetch(
        `${backendUrl}/usuarios/atualizar/${idUsuario}`,
        {
          method: "PATCH",
    ...
    if (Object.keys(enderecoUpdates).length > 0) {
      const enderecoResponse = await fetch(
        `${backendUrl}/enderecos/atualizar/${idEndereco}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
    ...
    document.querySelector(
      ".container_fundo_editar_informacoes"
    ).style.display = "none";
    showAlert(
    ...
    if (updates.senha) {
      ...
      try {
        ...
        const senhaResponse = await fetch(
          `${backendUrl}/reset-senha/nova-senha`,
          {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
